...
export default function RecipeReviewScreen() {
  ...
  const save = async () => { setSubmitted(true); setSaveError(undefined); if (validateRecipeDraft(draft).length) return; setSaving(true); try { const recipe = createImportedRecipe(draft, { url: job.source.url, label: job.source.host, creator: job.source.creator, platform: job.source.platform, externalId: job.source.externalId, imageUrl: job.source.imageUrl ?? job.source.localUri, mediaType: job.source.mediaType }); const saved = await addRecipe(recipe); await updateJob(job.id, { status: 'completed', recipeId: saved.id, recoveryCode: undefined }); router.replace(`/recipes/${saved.id}`); } catch (reason) { setSaveError(reason instanceof Error ? reason.message : 'Recipe could not be saved. Your import is still available.'); } finally { setSaving(false); } };
