...
export default function RecipeReviewScreen() {
  ...
  const save = async () => { setSubmitted(true); setSaveError(undefined); if (validateRecipeDraft(draft).length) return; setSaving(true); try { const recipe = createImportedRecipe(draft, { url: job.source.url, label: job.source.host, creator: job.source.creator, platform: job.source.platform, externalId: job.source.externalId, imageUrl: job.source.imageUrl ?? job.source.localUri, storagePath: job.source.storagePath, mediaType: job.source.mediaType }); const saved = await addRecipe(recipe); await updateJob(job.id, { status: 'completed', recipeId: saved.id, recoveryCode: undefined }); router.replace(`/recipes/${saved.id}`); } catch (reason) { setSaveError(reason instanceof Error ? reason.message : 'Recipe could not be saved. Your import is still available.'); } finally { setSaving(false); } };
  const keepForLater = async () => { setSaveError(undefined); setSaving(true); try { await updateJob(job.id, { status: 'needs_review', draft }); router.replace('/imports'); } catch (reason) { setSaveError(reason instanceof Error ? reason.message : 'Your progress could not be saved. Please try again.'); } finally { setSaving(false); } };
  return <Screen><KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.flex}><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.content}>
    ...
    <Card><Text style={styles.sourceLabel}>ORIGINAL SOURCE</Text><Text style={styles.source}>{job.source.host}</Text>{job.source.platform ? <Text style={styles.platform}>{job.source.platform[0].toUpperCase() + job.source.platform.slice(1)} import</Text> : null}{job.source.url ? <Text numberOfLines={2} style={styles.url}>{job.source.url}</Text> : <Text style={styles.url}>{job.source.storagePath ? 'Private cloud image attached' : 'Image kept on this device'}</Text>}</Card>
    {job.source.localUri || job.source.imageUrl ? <Image accessibilityLabel="Captured recipe reference" resizeMode="contain" source={{ uri: job.source.localUri ?? job.source.imageUrl }} style={styles.referenceImage} /> : null}
