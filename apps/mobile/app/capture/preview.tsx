...
export default function SourcePreviewScreen() {
  ...
  const start = async () => { if (!source || busy) return; setBusy(true); setStartError(undefined); try { const job = await createJob(source); router.replace({ pathname: '/capture/processing', params: { jobId: job.id } }); } catch (error) { setStartError(error instanceof Error ? error.message : 'We could not start this import.'); setBusy(false); } };
