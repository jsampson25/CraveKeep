...
export default function HouseholdMemberScreen() {
  ...
  const foodItems = useMemo(() => foodCategories.flatMap(category => category.items).filter((item, index, all) => all.indexOf(item) === index).filter(item => !query.trim() || item.toLowerCase().includes(query.trim().toLowerCase())).slice(0, 36), [query]);
  ...
  return <OnboardingShell title={title} percent={Math.round(((stepIndex + 1) / 6) * 100)} footer={<Button label={stepIndex === 5 ? 'Save member' : 'Continue'} onPress={next} />}>
