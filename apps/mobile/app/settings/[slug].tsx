...
const pages = {
  ...
  sources: { title: 'Connected sources', eyebrow: 'ACCOUNT & APP', icon: 'link-outline', intro: 'Choose a supported source and start bringing recipes into your private collection.', groups: [['YouTube', 'Import recipes from videos you save or share.'], ['Pinterest', 'Capture recipe pins into your collection.'], ['Websites and photos', 'Import from a recipe URL, image, or scan.']] },
...
export default function SettingsDetailScreen() {
  ...
  return <Screen><ScrollView contentContainerStyle={styles.content}>
    ...
    {storageError ? <Text accessibilityRole="alert" style={styles.error}>{storageError}</Text> : null}{page.groups.map(([title, detail], index) => { const action = actionFor(title); return <View key={title}><SectionTitle>{title}</SectionTitle><Pressable accessibilityRole={interactive ? 'switch' : action ? 'button' : undefined} disabled={!interactive && !action} onPress={action} style={!interactive && !action ? styles.disabledRow : undefined}><Card style={styles.row}><View style={styles.flex}><Text style={styles.rowTitle}>{title}</Text><Text style={styles.detail}>{detail}</Text></View>{interactive ? <Switch accessibilityLabel={title} onValueChange={(value) => setNotificationState((current) => current.map((item, itemIndex) => itemIndex === index ? value : item))} trackColor={{ false: colors.line, true: colors.coral }} thumbColor={colors.white} value={notificationState[index] ?? false} /> : action ? <Ionicons color={colors.muted} name="chevron-forward" size={19} /> : <Text style={styles.coming}>Planned</Text>}</Card></Pressable></View>})}
