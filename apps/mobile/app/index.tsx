...
const WELCOME_KEY = 'cravekeep.welcome.v4';
...
export default function Index() {
  ...
  return <View style={styles.screen}><Animated.View style={[styles.logoStage, { transform: [{ translateY: settle.interpolate({ inputRange: [0, 1], outputRange: [0, -270] }) }, { scale: settle.interpolate({ inputRange: [0, 1], outputRange: [1, 0.38] }) }] }]}><AnimatedBrandLogo assemble={assemble} word={word} /></Animated.View><Animated.View style={[styles.final, { opacity: settle }]}><View style={styles.art}><Image resizeMode="contain" source={foodOutline} style={[styles.food, styles.outline]} /><Animated.Image resizeMode="contain" source={foodColor} style={[styles.food, { opacity: food }]} /></View><Animated.View style={[styles.copy, { opacity: content }]}><Text accessibilityRole="header" style={styles.title}>Your recipes.{`\n`}Kept your way.</Text><Pressable accessibilityRole="button" onPress={() => void begin()} style={({ pressed }) => [styles.button, pressed && styles.pressed]}><Text style={styles.buttonText}>Let’s begin</Text></Pressable></Animated.View></Animated.View></View>;
}

const styles = StyleSheet.create({ screen: { flex: 1, backgroundColor: colors.paper, alignItems: 'center', justifyContent: 'center' }, logoStage: { position: 'absolute', width: 300, height: 286, alignItems: 'center', justifyContent: 'center', zIndex: 3 }, final: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, paddingHorizontal: spacing.lg, paddingTop: 126, paddingBottom: 36 }, art: { flex: 1, minHeight: 320, position: 'relative' }, food: { position: 'absolute', width: '100%', height: '100%' }, outline: { opacity: 0.42 }, copy: { gap: spacing.lg }, title: { color: colors.charcoal, fontFamily: 'Georgia', fontSize: 38, lineHeight: 43, fontWeight: '700', letterSpacing: -1 }, button: { minHeight: 58, borderRadius: radii.medium, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.coral }, pressed: { opacity: 0.8 }, buttonText: { color: colors.white, fontSize: 18, fontWeight: '800' } });
