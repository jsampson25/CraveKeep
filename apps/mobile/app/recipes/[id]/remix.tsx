...
export default function RemixRecipeScreen() {
  ...
  const save = async () => {
    ...
    try { const version = createRecipeVersion(original, draft, { goal, tasteProtection }); const saved = await addRecipe(version); router.replace(`/recipes/${saved.id}`); }
    catch (reason) { setSaveError(reason instanceof Error ? reason.message : 'The new version could not be saved. Please try again.'); }
