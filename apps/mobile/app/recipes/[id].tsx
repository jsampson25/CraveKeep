import { compareNutritionEstimates } from '@cravekeep/domain';
...
export default function RecipeDetailScreen() {
  ...
  useEffect(() => {
    let active = true;
    void fetchCookSessions(id, user?.id).then((sessions) => { if (active) { setCookSessions(sessions); setHistoryError(false); } }).catch(() => { if (active) setHistoryError(true); });
    return () => { active = false; };
  }, [id, user?.id]);
  ...
  const nutritionDifference = nutrition && originalNutrition ? compareNutritionEstimates(originalNutrition, nutrition) : undefined;
  const familyId = recipe.originalRecipeId ?? recipe.id;
  const versions = recipes.filter((item) => item.id === familyId || item.originalRecipeId === familyId).sort((a, b) => a.version - b.version);
  const latestCook = cookSessions[0];
  const averageTaste = cookSessions.length ? cookSessions.reduce((total, session) => total + session.taste, 0) / cookSessions.length : 0;
  const hasSourceVideo = recipe.source.mediaType === 'video' || recipe.source.mediaType === 'social';
  const openSource = () => { if (recipe.source.url) void WebBrowser.openBrowserAsync(recipe.source.url); };
  return <Screen><ScrollView contentContainerStyle={styles.content}>
    <View style={styles.header}><Pressable accessibilityLabel="Go back" onPress={() => router.back()} style={styles.circle}><Ionicons name="arrow-back" size={23} /></Pressable><View style={styles.actions}>{recipe.source.kind === 'manual' && recipe.version === 1 && !recipe.originalRecipeId ? <Pressable accessibilityLabel="Edit recipe" onPress={() => router.push(`/recipes/${recipe.id}/edit`)} accessibilityRole="button" style={styles.circle}><Ionicons color={colors.charcoal} name="create-outline" size={23} /></Pressable> : null}<Pressable accessibilityLabel={recipe.favorite ? 'Remove favorite' : 'Favorite recipe'} onPress={() => toggleFavorite(recipe.id)} accessibilityRole="button" style={styles.circle}><Ionicons color={recipe.favorite ? colors.coral : colors.charcoal} name={recipe.favorite ? 'heart' : 'heart-outline'} size={23} /></Pressable></View></View>
    <RecipeArt favorite={recipe.favorite} imageUrl={recipe.source.imageUrl} storagePath={recipe.source.storagePath} /><Title>{recipe.title}</Title><Text style={styles.body}>{recipe.description}</Text>
    ...
    <Card><Text style={styles.sourceLabel}>SOURCE</Text><Text style={styles.source}>{recipe.source.label}</Text>{recipe.source.platform ? <Text style={styles.body}>{recipe.source.platform[0].toUpperCase() + recipe.source.platform.slice(1)} import{recipe.source.creator ? ` · ${recipe.source.creator}` : ''}</Text> : recipe.source.creator ? <Text style={styles.body}>By {recipe.source.creator}</Text> : null}{recipe.source.url ? <Pressable accessibilityRole="button" onPress={openSource} style={styles.sourceAction}><Ionicons color={colors.coralDark} name="open-outline" size={17} /><Text style={styles.nutritionLink}>Open original source</Text></Pressable> : null}</Card>
    <Card><Text style={styles.sourceLabel}>YOUR COOKING HISTORY</Text>{latestCook ? <><View style={styles.historySummary}><View><Text style={styles.source}>{cookSessions.length} cook{cookSessions.length === 1 ? '' : 's'}</Text><Text style={styles.body}>{averageTaste.toFixed(1)} / 5 average taste</Text></View><View style={styles.repeatBadge}><Ionicons color={latestCook.repeatIntent ? colors.herb : colors.coralDark} name={latestCook.repeatIntent ? 'refresh-circle' : 'create-outline'} size={20} /><Text style={latestCook.repeatIntent ? styles.private : styles.versionText}>{latestCook.repeatIntent ? 'Make again' : 'Adjust first'}</Text></View></View><Text style={styles.historyDate}>Latest · {new Date(latestCook.cookedAt).toLocaleDateString()} · {latestCook.effort === 'expected' ? 'Effort as expected' : latestCook.effort === 'easy' ? 'Easier than expected' : 'Harder than expected'}</Text>{latestCook.notes ? <Text style={styles.historyNotes}>“{latestCook.notes}”</Text> : null}</> : <Text style={styles.body}>{historyError ? 'History could not be refreshed from the cloud. Your device data is unchanged.' : 'Finish Cook Mode to start your private cooking history.'}</Text>}</Card>
    {hasSourceVideo ? <Button label="Watch source video" variant="secondary" onPress={() => router.push(`/recipes/${recipe.id}/video` as never)} /> : null}
