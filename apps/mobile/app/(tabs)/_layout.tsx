...
export default function TabLayout() {
  const { ready, user } = useAuthStore(); const { ready: onboardingReady, profile } = useOnboardingStore(); const { width } = useWindowDimensions();
  ...
  return <Tabs screenOptions={({ route }) => ({ headerShown: false, tabBarActiveTintColor: colors.coral, tabBarInactiveTintColor: colors.muted, tabBarStyle: styles.bar, tabBarLabelStyle: styles.label, tabBarIcon: ({ color, focused }) => <Ionicons color={color} name={iconFor(route.name, focused)} size={22} /> })}>
    ...
    <Tabs.Screen name='capture-button' options={{ title: '', tabBarButton: () => <View pointerEvents='box-none' style={[styles.captureWrap, { left: width / 2 - 29 }]}><Pressable accessibilityLabel='Open Capture Studio' accessibilityRole='button' onPress={() => router.push('/capture')} style={styles.capture}><Ionicons color={colors.white} name='add' size={30} /></Pressable></View> }} />
...
const styles = StyleSheet.create({
  ...
  label: { fontSize: 11, fontWeight: '700' }, captureWrap: { position: 'absolute', top: 0, width: 58, height: 64, alignItems: 'center', zIndex: 20 },
