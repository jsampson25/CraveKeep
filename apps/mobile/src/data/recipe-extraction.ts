import type { CaptureJob, SourcePreview } from '@cravekeep/domain';
...
export async function extractRecipeFromLink(url: string, title: string): Promise<Pick<CaptureJob, 'status' | 'draft' | 'warnings' | 'recoveryCode' | 'source'> & { source?: Partial<SourcePreview> }> {
  if (!supabase) throw new Error('Cloud extraction is not configured.');
