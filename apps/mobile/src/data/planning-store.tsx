...
export function PlanningStoreProvider({ children }: PropsWithChildren) {
  ...
  useEffect(() => { if (!ready || !user) return; void fetchCloudPlan(user.id).then((cloud) => { setPlan((local) => { const next = { targets: cloud.targets ?? local.targets, meals: [...cloud.meals, ...local.meals.filter((meal) => !cloud.meals.some((remote) => remote.id === meal.id))] }; void AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(next)).catch(() => setError('Planning refreshed, but the local cache could not be updated.')); return next; }); }).catch(() => setError('Cloud planning data could not be refreshed.')); }, [ready, user]);
  const persist = useCallback(async (next: StoredPlan) => { setPlan(next); try { await AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(next)); } catch { setError('Your planning change is visible but could not be saved on this device.'); } }, []);
